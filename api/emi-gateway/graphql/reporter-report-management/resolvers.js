...
module.exports = {
    ...
    //// QUERY ///////
    Query: {
        ReporterReportListing(root, args, context) {
            return sendToBackEndHandler$(root, args, context, READ_ROLES, 'query', 'Report', 'ReporterReportListing').toPromise();
        },
    ...
    //// MUTATIONS ///////
    Mutation: {
        ReporterCreateReport(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'Report', 'ReporterCreateReport').toPromise();
        },
        ReporterUpdateReport(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'Report', 'ReporterUpdateReport').toPromise();
        },
        ReporterDeleteReports(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'Report', 'ReporterDeleteReports').toPromise();
        },
    },

    //// SUBSCRIPTIONS ///////
    Subscription: {
        ReporterReportModified: {
            subscribe: withFilter(
                (payload, variables, context, info) => {
                    ...
                    return pubsub.asyncIterator("ReporterReportModified");
                },
                (payload, variables, context, info) => {
                    return payload
                        ? (payload.ReporterReportModified.id === variables.id) || (variables.id === "ANY")
                        : false;
                }
            )
        }
